...
db = MySQLDatabase('email_messages', user='root', passwd='')


class Email(peewee.Model):
    # author = peewee.CharField()
    # title = peewee.TextField()
    sent_time= peewee.DateTimeField()
    message_folder =peewee.TextField()
    message_body = peewee.TextField()
    message_mailbox=peewee.TextField()
    recipient_address=peewee.TextField()
    message_id=peewee.TextField()
    email_attachment=peewee.TextField()
    received_time=peewee.DateTimeField()
    sender_address=peewee.TextField()
    message_subject=peewee.TextField()
    recipient_count=peewee.IntegerField()
    class Meta:
        database = db
...
#         print 'skipping because of folder'
    ...
    ormemail = Email(sent_time=dateutil.parser.parse(headersdict["Date"]),
                message_folder=subFolder,
                message_body=msg._payload,
                message_mailbox=mailboxOwner,
                recipient_address=headersdict["To"],
                message_id=headersdict["Message-ID"],
                email_attachment='',
                received_time=dateutil.parser.parse(headersdict["Date"]),
                sender_address=headersdict["From"],
                message_subject=headersdict["Subject"],
                recipient_count=len(headersdict["To"].split(','))
                )
