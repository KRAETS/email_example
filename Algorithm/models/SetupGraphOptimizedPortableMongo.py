...
def setupGraph(nodestopnum=None):
    ...
    #REPLACEMENT
    aexp="SELECT DISTINCT \ALL*email_address*_:source\ FROM \ALL/emailmessage}\ WHERE ( \ALL*folder\ = 'sent_items' OR \ALL*folder\ = 'sent')"
    aexpontology = "src/main/resources/jsonemailsql/"
    outputfile = "output/outfile"
    callparams = ["java", "-jar", "./kql_engine-1.1-SNAPSHOT-jar-with-dependencies.jar", '-kqlq', aexp, '-d',
    '-ont', aexpontology, '-fol', '0', '-out', outputfile, '-u', 'dbuser', '-p', 'dbuser', '-url',
                  "jdbc:mongo://localhost:27017/enron_mail_clean?rebuildschema=true"]
    s = call(callparams)
    c1= c1+1
    #print "Done"
...
#         print nodenum, len(nodelist1), len(G.vs), len(G.es)
    ...
    aexpontology = "src/main/resources/jsonemailsql/"
    outputfile = "output/outfile"
    callparams = ["java", "-jar", "./kql_engine-1.1-SNAPSHOT-jar-with-dependencies.jar",
                  '-ont', aexpontology, '-fol', '0', '-out', outputfile, '-u', 'dbuser', '-p', 'dbuser', '-url',
                  "jdbc:mongo://localhost:27017/enron_mail_clean?rebuildschema=true",'-ql']
    finalarglist =callparams+querylist
    call(finalarglist)
